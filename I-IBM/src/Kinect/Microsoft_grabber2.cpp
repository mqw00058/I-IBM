...
void KinectGrabber::start() {
	...
	pCameraSpacePoints = new CameraSpacePoint[cColorWidth*cColorHeight];
	memset(pCameraSpacePoints, 0, sizeof(CameraSpacePoint) * cColorWidth*cColorHeight);
	mCameraSpacePoint = Mat(cColorHeight, cColorWidth, CV_32FC3, pCameraSpacePoints);
...
void KinectGrabber::GetNextFrame() {
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			//  memcpy(pDepthBuffer, m_pDepthBuffer, sizeof(UINT16) * cDepthHeight*cDepthWidth);
		...
		hr = m_pCoordinateMapper->MapColorFrameToCameraSpace(
			cDepthHeight*cDepthWidth,
			m_pDepthBuffer,
